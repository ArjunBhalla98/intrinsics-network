...
def plot_logs(prefix, ax, labels):
    ...
    if "normals" not in labels:
        ax.plot(
            x, [-0.005 for i in x], linewidth=3, color=colors["normals"], label="Shape"
        )

    ax.tick_params(
        axis="both", which="both", top="off", bottom="off", left="off", right="off"
    )
    # ax = plt.subplot(111)
...
cyan = [112, 173, 246]
colors = {
    "albedo": make_color(red),
    "normals": make_color(green),
    "shading": make_color(blue),
    "render": make_color(cyan),
}
fig_labels = {
    "albedo": "Reflectance",
    "normals": "Shape",
...
lgd = plt.legend(h, l, bbox_to_anchor=(1.2, -0.225), fontsize=12, ncol=4, frameon=False)
fig.set_size_inches(6, 4)
plt.savefig(
    "plots/errors_normed.png",
    bbox_extra_artists=(
        sup,
        lgd,
    ),
