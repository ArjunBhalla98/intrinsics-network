...
class IntrinsicDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.datasets = [
            os.path.join(directory, dataset) for dataset in datasets.split(",")
        ]
        ...
        for dataset in self.datasets:
            ...
            for sel in self.selections:
                ...
                if sel == "lights":
                    ## load lights array
                    files = np.load(
                        os.path.join(self.rel_path, "dataset/arrays/", array + ".npy")
                    )[:size_per_dataset, :]
            ...
            for ind, sel in enumerate(self.selections):
                ...
                if sel == "lights":
                    ...
                    self.data_files[ind] = {
                        ...
                        for key in self.data_files[ind].keys()
                    }
                else:
    ...
    def __find_sort_files(self, dataset, sel):
        files = [
            fname
            for fname in os.listdir(self.rel_path + dataset)
