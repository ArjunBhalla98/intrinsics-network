...
def visualize_shader(model, loader, save_path, save_raw=False):
    ...
    for ind, tensors in enumerate(loader):
        ...
        ## shape inputs
        inputs.extend(
            [pipeline.vector_to_image(img.squeeze()) for img in inp[0].data.split(1)]
        )
        ## shading predictions
...
def visualize_composer(model, loader, save_path, epoch, raw=False):
    ...
    for ind, tensors in enumerate(loader):
        ...
        depth_normals_targ = pipeline.depth_to_normals(
            depth_targ.unsqueeze(1), mask=mask
        )
        # depth_normals_targ
