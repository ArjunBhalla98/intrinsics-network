...
class ComposerDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        inds=None,
        unlabeled_array="shader2",
        labeled_array="shader2",
    ):
        ...
        self.unlabeled_datasets = [
            ...
            for dataset in unlabeled_datasets.split(",")
        ]
        self.labeled_datasets = [
            os.path.join(directory, dataset) for dataset in labeled_datasets.split(",")
        ]
        ...
        self.unlabeled_data_files = self.__find_dataset(
            self.unlabeled_datasets,
            self.unlabeled_selections,
            size_per_dataset,
            inds,
            self.unlabeled_array,
        )
        self.labeled_data_files = self.__find_dataset(
            self.labeled_datasets,
            self.labeled_selections,
            size_per_dataset,
            inds,
    ...
    def __find_dataset(self, datasets, selections, size_per_dataset, inds, array):
        ...
        for dataset in datasets:
            ...
            for sel in selections:
                ...
                ## reflectance, shading
                    ...
                    files = {}
                    files["reflectance"] = self.__find_sort_files(dataset, "albedo")[
                        :size_per_dataset
                    ]
        ...
        if inds:
            inds = [
                i + (offset * self.size_per_dataset)
            ...
            for ind, sel in enumerate(selections):
                ...
                if sel == "lights":
                    ...
                    data_files[ind] = {
                        key: [files[key][i] for i in inds]
                        for key in data_files[ind].keys()
                    }
                else:
    ...
    def __find_sort_files(self, dataset, sel):
        files = [
            ...
            if sel + ".png" in fname and "sphere" not in fname
        ]
    ...
    def __getitem__(self, unlabeled_idx):
        # print idx
        unlabeled_outputs = self.__read_data_files(
            self.unlabeled_data_files, self.unlabeled_selections, unlabeled_idx
        )

        ## just grab a random labeled example
        labeled_idx = np.random.randint(self.labeled_size)
        labeled_outputs = self.__read_data_files(
            self.labeled_data_files, self.labeled_selections, labeled_idx
        )
...
if __name__ == "__main__":
    ...
    loader = torch.data.DataLoader(dset, batch_size=16, num_workers=4)
    print("done init")
    time_0 = time.time()
    for i, inp in enumerate(loader):
        ...
        print(
            i,
            [[t.size() for t in sublist] for sublist in inp],
            len(inp),
            len(inp[0]),
            len(inp[1]),
        )
