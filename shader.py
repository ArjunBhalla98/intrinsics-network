...
train_set = pipeline.IntrinsicDataset(
    args.data_path, args.train_sets, args.intrinsics, size_per_dataset=args.num_train
)
...
val_set = pipeline.IntrinsicDataset(
    args.data_path, args.val_sets, args.intrinsics, size_per_dataset=10
)
val_loader = torch.utils.data.DataLoader(
    val_set, batch_size=32, num_workers=4, shuffle=False
)
...
for epoch in range(args.num_epochs):
    ...
    torch.save(
        shader.state_dict(), open(os.path.join(args.save_path, "state.pth"), "wb")
    )
