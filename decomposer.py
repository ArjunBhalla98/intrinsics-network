...
parser.add_argument(
    "--data_path", type=str, default="dataset/output/", help="base path for datasets"
)
parser.add_argument(
    "--train_sets",
    type=str,
    default="motorbike_train,bottle_train",
    help="folders within data_path to draw from during training",
)
parser.add_argument(
    "--val_sets",
    type=str,
    default="motorbike_val,bottle_val",
    help="folders within data_path to draw from during validation",
)
parser.add_argument(
    "--intrinsics",
    type=list,
    default=["input", "mask", "albedo", "depth", "normals", "lights"],
    help="intrinsic images to load from the train and val sets",
)
parser.add_argument(
    "--save_path",
    type=str,
    default="components/test_logger/",
...
parser.add_argument(
    "--lights_mult", type=float, default=0.01, help="multiplier on the lights loss"
)
parser.add_argument(
    "--array", type=str, default="shader", help="array with lighting parameters"
)
parser.add_argument(
    "--num_train",
    type=int,
...
parser.add_argument(
    "--loaders", type=int, default=4, help="number of parallel data loading processes"
)
...
train_loader = torch.utils.data.DataLoader(
    train_set, batch_size=args.batch_size, num_workers=args.loaders, shuffle=True
)
...
val_loader = torch.utils.data.DataLoader(
    val_set, batch_size=args.batch_size, num_workers=args.loaders, shuffle=False
)
