import math, numpy as np, imageio, pdb
...
# pdb.set_trace()
img[:, :, : channels - 1] = img[:, :, : channels - 1] / 2.0 + 0.5
print(img.max(0).max(0))
print(img.min(0).min(0))
imageio.imsave("normals_vector.png", img)
...
for ind in range(channels - 1):
    ...
    for mask in range(channels - 1):
        if mask != ind:
            copy[:, :, mask] = 0
    imageio.imsave("mask_" + str(ind) + ".png", copy)
